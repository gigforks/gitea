...
func (u *User) GetOrgRepositoryIDs() ([]int64, error) {
	var ids []int64
	return ids, x.Table("team_repo").
		Cols("team_repo.repo_id").
		Join("INNER", "team_user", "team_repo.team_id = team_user.team_id AND team_user.uid = ?", u.ID).
		GroupBy("team_repo.repo_id").Find(&ids)
}
...
// GetAccessibleRepositoriesIds gets all ids for the public/private repos that user has access to
func (user *User) GetAccessibleRepositoriesIds() ([]int64, error) {
	sess := x.NewSession()
	defer sess.Close()
	var repos_ids []int64

	if user.IsAdmin {
		err := sess.Select("id").Table("repository").Find(&repos_ids);
		if err != nil {
	...
	cond = cond.Or(builder.Eq{"owner_id": user.ID}, builder.Eq{"is_private": false})
